...
export const registerBrand = asyncHandler(async (req, res, next) => {
  ...
  // 4. Call Service
  const brand = await BrandsService.registerBrand(
    req.user.id,
    {
...
export const getBrandsByLocation = asyncHandler(async (req, res, next) => {
  ...
  const result = await BrandsService.getBrandsNearby(
    lat,
    lng,
    rad,
    pageNumber,
...
export const updateBrand = asyncHandler(async (req, res, next) => {
  ...
  let logoUrl: any = undefined;
  let bannerUrl: any = undefined;

  const files = req.files as { [fieldname: string]: Express.Multer.File[] } | undefined;

  if (files) {
    if (files["logo"]?.[0]) {
      const oldLogo = existingBrand.logoUrl as any;
    ...
    if (files["banner"]?.[0]) {
      const oldBanner = existingBrand.bannerUrl as any;
  ...
  const brandData: any = {};
  if (validated.brandName !== undefined) brandData.brandName = validated.brandName;
  if (validated.slug !== undefined) brandData.slug = validated.slug;
  if (validated.description !== undefined) brandData.description = validated.description;
  if (logoUrl !== undefined) brandData.logoUrl = logoUrl;
  if (bannerUrl !== undefined) brandData.bannerUrl = bannerUrl;
  if (validated.contactEmail !== undefined) brandData.contactEmail = validated.contactEmail;
  if (validated.contactPhone !== undefined) brandData.contactPhone = validated.contactPhone;
  if (validated.address !== undefined) brandData.address = validated.address;
  if (validated.gstNumber !== undefined) brandData.gstNumber = validated.gstNumber;
  if (validated.isActive !== undefined) brandData.isActive = validated.isActive;

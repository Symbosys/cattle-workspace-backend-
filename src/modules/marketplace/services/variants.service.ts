...
export class VariantsService {
  ...
  public static async updateVariant(
    ...
    return await db.$transaction(async (tx) => {
      ...
      if (data.imageUrl !== undefined) {
        const oldImage = variant.imageUrl as any;
      ...
      if (data.imageUrl !== undefined) updateData.imageUrl = data.imageUrl as any;
   ...
   */
  public static async deleteVariant(ownerId: string, id: string) {
    return await db.$transaction(async (tx) => {
      // 1. Fetch variant and verify ownership
      const variant = await VariantsRepository.findById(id, tx);
      if (!variant) {
   ...
   * Get variant by ID
   */
  public static async getVariantById(id: string) {

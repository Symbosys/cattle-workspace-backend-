...
export class ReviewsService {
  ...
  public static async updateReview(
    ...
    return await db.$transaction(async (tx) => {
      ...
      if (data.images !== undefined) {
        const oldImages = (review.images as any) || [];
      ...
      // 4. Perform update
      const updateData: Prisma.ProductReviewUncheckedUpdateInput = {};
   ...
   */
  public static async addSellerReply(
    ownerId: string,
    reviewId: string,
    reply: string
  ) {
    return await db.$transaction(async (tx) => {
      ...
      return await ReviewsRepository.update(
        reviewId,
        {
          sellerReply: reply,
        },
  ...
  public static async deleteReview(userId: string, reviewId: string) {
    return await db.$transaction(async (tx) => {
      ...
      const reviewImages = (review.images as any) || [];

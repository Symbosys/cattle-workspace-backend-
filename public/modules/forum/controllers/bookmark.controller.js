...
import { toggleBookmarkSchema, listBookmarksSchema, } from "../schema/bookmark.schema.js";
...
export const toggleBookmark = asyncHandler(async (req, res, next) => {
    ...
    const result = await BookmarkService.toggleBookmark(req.user.id, validated.questionId);
...
export const listUserBookmarks = asyncHandler(async (req, res, next) => {
    ...
    const result = await BookmarkService.listUserBookmarks(req.user.id, validated);
    return SuccessResponse(res, "Bookmarks fetched successfully", result, 200);
});
/**
...
export const checkBookmarkStatus = asyncHandler(async (req, res, next) => {
    ...
    const result = await BookmarkService.isBookmarked(req.user.id, questionId);
    return SuccessResponse(res, "Bookmark status fetched successfully", result, 200);
});

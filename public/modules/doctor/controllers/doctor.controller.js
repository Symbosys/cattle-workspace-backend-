...
export const registerDoctor = asyncHandler(async (req, res, next) => {
    ...
    // 4. Register doctor
    const doctor = await DoctorService.registerDoctor(req.user.id, {
    ...
    return SuccessResponse(res, "Doctor registered successfully", doctor, 201);
});
...
export const updateDoctor = asyncHandler(async (req, res, next) => {
    ...
    // 6. Perform update
    const updatedDoctor = await DoctorService.updateDoctor(req.user.id, id, doctorData, locationData);
    return SuccessResponse(res, "Doctor profile updated successfully", updatedDoctor, 200);
});
export const getDoctorsByLocation = asyncHandler(async (req, res, next) => {
    ...
    const result = await DoctorService.getDoctorsByLocation(lat, lng, rad, pageNumber, pageSize);

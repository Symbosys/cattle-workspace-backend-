...
export const getDoctorReviews = asyncHandler(async (req, res, next) => {
    ...
    const result = await ReviewService.getDoctorReviews(doctorId, validated.page, validated.limit);
...
export const getReviewByAppointmentId = asyncHandler(async (req, res, next) => {
    ...
    const review = await ReviewService.getReviewByAppointment(req.user.id, appointmentId);
    if (!review) {
